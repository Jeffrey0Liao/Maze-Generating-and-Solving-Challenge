...
function exp_array = expand(node_x, node_y, gn, xTarget, yTarget, OBSTACLE, MAX_X, MAX_Y, maze)
    exp_array = []; % children nodes
    exp_count = 1;
    c2 = size(OBSTACLE, 1);
    for k = 1 : -1 : -1 % explore surrounding locations
        for j = 1 : -1 : -1
            if ((k ~= j || k ~= 0)&&(k==0||j==0))  % the node itself is not its successor also the adjency
                s_x = node_x + k;
                s_y = node_y + j;
